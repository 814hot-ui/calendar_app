      ...
      body: ListView(
        children: [
          ...
          ListTile(
            ...
            onTap: () {
              showModalBottomSheet(
                context: context,
                builder: (modalContext) { // 👈 변수명을 modalContext로 명확히 분리합니다.
                  // ⭕ 핵심: 바텀시트 내부에서도 부모의 settings(Provider)를 정확히 바라보도록 재선언합니다.
                  final currentSettings = Provider.of<SettingsManager>(context, listen: false);
              ...
              showDialog(
                ...
                builder: (context) => AlertDialog(
                  ...
                  content: SingleChildScrollView(
                    child: ColorPicker( // ⚡ BlockPicker를 대형 ColorPicker로 교체!
          ...
          ListTile(
            ...
            subtitle: Text('backup_export_subtitle'.tr()), // 🌐 "현재 일기 데이터를 파일로 안전하게 백업합니다."
            onTap: () async {
              // ⭕ ScaffoldMessenger를 내부에 직접 명시하여 에러를 차단합니다.
              await BackupService.instance.exportBackup((msg) {
          ...
          ListTile(
            ...
            subtitle: Text('backup_import_subtitle'.tr()), // 🌐 "백업된 파일을 가져와 기존 일기 데이터에 덮어씁니다."
            onTap: () async {
              // ⭕ ScaffoldMessenger를 내부에 직접 명시하여 에러를 차단합니다.
          ...
          FutureBuilder<PackageInfo>(
            ...
            builder: (context, snapshot) {
              if (snapshot.hasData) {
                ...
                return Padding(
                  padding: const EdgeInsets.all(16.0), // 👈 대메뉴 타이틀과 정확히 일치하는 여백
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start, // 👈 왼쪽 라인 칼정렬
                    children: [
                      // 1. 버전 정보
                      Text(
                      ...
                      const Text(
                        ...
                        style: TextStyle(
                          color: Colors.grey,
                          fontSize: 12, // 💡 저작권은 한 단계 작게 들어가야 UI가 훨씬 세련되게 떨어집니다.
